...
class Datalist:
    def __init__(self, trained_datalist=None, word2id=None):
        """
        ...
        """
        self.word2vec = word2id
        self.train = trained_datalist is None
        if self.train:
            self.word_nums = Numberer()  # word2id if word2id else Numberer()
...
class AspectDatalistBase(Datalist):
    ...
    def __init__(self, trained_datalist=None, word2id=None):
        super().__init__(trained_datalist, word2id)
    ...
    def process_document_text(self, document):
        ...
        pos_tags = [[1] for o in sentences]  # self.pos_tagger.tag_sents(sentences)
        ...
        for sentence, sentence_pos in zip(sentences, pos_tags):
            if not self.word2vec:
                ...
                numbered_sentences += [
                    self.word_nums[word] for word in sentence
                ]
            numbered_pos_tags += [
                self.word_nums.number(word.lower(), self.train) for word in sentence
            ]
    ...
    def create_iob_batches(self, iob_data, batch_size, mini_batch_size, mini_batch=True, bucketing=False,
                           predict=False):
        if bucketing:
        ...
        for start, end in zip(
            ...
            aspect_batch = np.zeros((batch_size, self.category_nums.max, self.emo_nums.max), dtype=np.int32)
            if not predict:
            ...
            for document_index, (
            document, iob_markup, pos_tags, single_length, document_length, polarity, category) in enumerate(
                    iob_data[start:end]):
                ...
                category = [self.type2BI.get(self.polarity_aspect_category_nums.num2value.get(cat,1),{1})
                            for cat in category]

                for pol,cat in zip(polarity, category):
                    aspect_batch[document_index, list(cat), pol] = 1
                ...
                if not predict:
                    for i, iob in enumerate(iob_markup[:min(document_length, self.max_len_sentences)]):
                        if iob:  # if there are aspect annotations on this token
                            ...
                            doc_set.update([0])
                iob_cnt.update(doc_set)
                num_doc += 1
            if mini_batch:
                if predict:
                    iob_batch = None
                iob_batch, text_batch, pos_batch, document_lengths, aspect_batch = self.create_mini_batch(batch_size,
                                                                                                          iob_batch,
                                                                                                          text_batch,
                                                                                                          pos_batch,
                                                                                                          aspect_batch,
            ...
            aspect_batches.append(aspect_batch)

        return (text_batches, iob_batches, document_length_batches, pos_batches, aspect_batches), iob_cnt, num_doc if not predict else (
        text_batches, document_length_batches, pos_batches)

    def create_mini_batch(self, batch_size, iob, text, pos, aspect_batch, sentence_lengths, max_length):
        ...
        if not prediction:
            ...
            aspect_new = np.zeros([n_splits, batch_size, self.category_nums.max, self.emo_nums.max], dtype=np.int32)
        ...
        for x, (single_breakpoints, single_length, single_text, single_iob, single_pos, aspect) \
                in enumerate(zip(ceiled_cum_sum, sentence_lengths, text, iob, pos, aspect_batch)):
            ...
            for i in range(0, n_splits):
                ...
                if not prediction:
                    ...
                    cat = np.sum(iob_new[i, x, :copy_until], axis=0) > 0
                    aspect_new[i, x, cat] = aspect[cat]

                pos_new[i, x, :copy_until] = single_pos[i * copy_until:(i + 1) * copy_until]
                length_new[i, x] = copy_until

        return iob_new, text_new, pos_new, length_new, aspect_new
    ...
    @property
    def n_words(self):
        return self.word_nums.max  # if not self.word2vec else len(self.word2vec)
...
class AspectDatalist(AspectDatalistBase):
    def __init__(self, trained_datalist=None, word2id=None):
        super().__init__(trained_datalist, word2id=word2id)
    ...
    def load_iob(self, path, verbose=False):
        ...
        for processed_count, element in enumerate(parser.xpath("//Document"), 1):
            ...
            sentences, numbered_sentences, numbered_pos_tags, sentence_lengths, single_lengths = self.process_document_text(
                element_text)
            ...
            for opinion in element.xpath(".//Opinion"):
                ...
                if target == "NULL":
                    ...
                    if not annotation in annotation_to_index:
                        ...
                        self.cat_counter[self.polarity_aspect_category_nums.number(category, self.train)] += 1
                        aspect_categories.append(self.polarity_aspect_category_nums.number(category, self.train))
                    else:
                        aspect_categories.append(self.polarity_aspect_category_nums.number(category, self.train))
                            ...
                            for categories in available_ends.values():
                                for category in categories:
                                    ...
                                    if category[1][1]:  # if the annotation doesn't have a begin element yet
                                        ...
                                        try:
                                            self.type2BI[category[1][0]].add(self.category_nums.number(
                                                (IOB_Type.B, category[1][0]), self.train))
                                        except KeyError:
                                            self.type2BI[category[1][0]] = {self.category_nums.number(
                                                (IOB_Type.B, category[1][0]), self.train)}
                                    else:
                                        iob_annotation[-1].append(
                                        ...
                                        try:
                                            self.type2BI[category[1][0]].add(self.category_nums.number(
                                                (IOB_Type.I, category[1][0]), self.train))
                                        except KeyError:
                                            self.type2BI[category[1][0]] = {self.category_nums.number(
                                                (IOB_Type.I, category[1][0]), self.train)}
            ...
            self.iob_data.append((numbered_sentences, iob_annotation, numbered_pos_tags, single_lengths,
                                  sentence_lengths, aspect_polarities, aspect_categories))
    ...
    def create_aspect_polarity_batches(self, batch_size):
        ...
        for start, end in zip(
            ...
            for document_index, (document, aspect_markup, polarity, aspect_category, pos_tags) in enumerate(
                    self.polarity_data[start:end]):
