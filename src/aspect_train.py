...
def train_iob_model(training_batches, validation_batches, training_max_length, validation_max_length, config):
    ...
    with tf.Session() as session:
        ...
        for epoch in range(config.max_epochs):
            ...
            hamming_loss = 0
            for mini_text_batch, mini_category_batch, mini_length_batch, mini_pos_batch in zip(*training_batches):
            ...
            predicted_positives = np.zeros([config.label_size])
            true_negatives = np.zeros([config.label_size])
            true_positives = np.zeros([config.label_size])
            false_negatives = np.zeros([config.label_size])

            for mini_text_batch, mini_category_batch, mini_length_batch, mini_pos_batch in zip(*validation_batches):
                mini_batch_hamming = 0
                ...
                for text, cat, length, pos in zip(mini_text_batch, mini_category_batch, mini_length_batch,
                    ...
                    (fw_init_state, bw_init_state), loss, accuracy, hamming, predictions_vs_labels, \
                    true_pos, \
                    false_pos, \
                    true_neg, \
                    false_neg = session.run([validation_model.state, validation_model.loss, validation_model.accuracy,
                                             validation_model.ham, validation_model.label_equality,
                                             ...
                                             validation_model.true_neg, validation_model.false_neg],
                                            {
                                                ...
                                                validation_model.labels: cat,
                                                validation_model.document_lengths: length,
                                                validation_model.pos: pos
                                            }
                                            )
                    mini_batch_hamming += hamming
                    true_positives += np.sum(np.reshape(true_pos, [-1, config.label_size]), axis=0)
                    predicted_positives += predictions_vs_labels[1]
                    true_negatives += np.sum(np.reshape(true_neg, [-1, config.label_size]), axis=0)
...
def train_aspect_polarity_model(training_batches, validation_batches, training_max_length, validation_max_length,
    ...
    with tf.Session() as session:
        ...
        for epoch in range(config.max_epochs):
            ...
            for mini_text_batch, mini_annotation_batch, mini_polarity_batch, mini_length_batch, mini_category_batch, mini_pos_batch in zip(
                    *training_batches):
                fw_init_state = np.zeros([config.batch_size, config.hidden_neurons])
                bw_init_state = np.zeros([config.batch_size, config.hidden_neurons])
                ...
                for text_batch, annotation_batch, polarity_batch, length_batch, category_batch, pos_batch in \
                        zip(mini_text_batch, mini_annotation_batch, mini_polarity_batch, mini_length_batch,
                            mini_category_batch, mini_pos_batch):
                    (fw_init_state, bw_init_state), loss, _ = session.run(
                        [train_model.state, train_model.loss, train_model.training_operation],
                        {
...
def parse_arguments():
    ...
    polarity_parser.add_argument('--mini-batch-size', '-mb', metavar='N', type=int, default=150,
...
if __name__ == '__main__':
    ...
    if arguments.model == "iob":
        ...
        training_batches, validation_batches = load_aspect_polarity_batches(train_datalist, validation_datalist,
                                                                            config.batch_size, config.mini_batch_size)
        # ESC[2K clears the line
        print("\x1b[2KTraining started")
