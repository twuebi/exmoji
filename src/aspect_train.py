...
IOBConfig = namedtuple("IOBConfig",
                       ...
                       "patience model_path n_polarities"
                       )
...
def train_iob_model(training_batches, validation_batches, training_max_length, validation_max_length, config,
                    count=None, num_docs=0):
    label_freq = np.array([count[i] for i in range(len(count))])
    a = np.argwhere(label_freq != 0)
    idf = np.zeros([config.label_size])
    idf[a] = np.expand_dims(np.log(num_doc / label_freq[label_freq != 0]), 1)

    with tf.Session() as session:
        # embeddings = gensim.models.KeyedVectors.load_word2vec_format('german.model', binary=True).syn0
        with tf.variable_scope("model", reuse=False):
            train_model = IOBModel(config, training_max_length, Mode.TRAIN)
            # session.run([train_model.emb_init],feed_dict={train_model.emb_placeholder:embeddings})
        ...
        for epoch in range(config.max_epochs):
            ...
            hamming = 0
            for mini_text_batch, mini_iob_batch, mini_length_batch, mini_pos_batch, mini_aspect_batch in zip(*training_batches):
                ...
                mini_batch_loss = 0
                for text, iob, length, pos, aspect in zip(mini_text_batch, mini_iob_batch, mini_length_batch, mini_pos_batch, mini_aspect_batch):
                    ratio = (np.sum(np.count_nonzero(iob, axis=0), axis=0)) / np.sum(length)
                    ratio = ratio * idf
                    # ratio[np.argwhere(ratio!=0)] = 1/ratio[np.argwhere(ratio!=0)]

                    (fw_init_state, bw_init_state), loss, _,log = session.run(
                        [train_model.state, train_model.loss, train_model.training_operation, train_model.logits],
                        {
                            train_model.aspects: aspect,
            ...
            for mini_text_batch, mini_iob_batch, mini_length_batch, mini_pos_batch, mini_aspect_batch in zip(*validation_batches):
                mini_batch_accuracy = 0
                mini_batch_loss = 0
                fw_init_state = np.zeros([config.batch_size, config.hidden_neurons])
                bw_init_state = np.zeros([config.batch_size, config.hidden_neurons])
                for text, iob, length, pos, aspect in zip(mini_text_batch, mini_iob_batch, mini_length_batch,
                                                          mini_pos_batch, mini_aspect_batch):
                    ratio = np.ones([config.label_size])
                    (fw_init_state, bw_init_state), loss, accuracy , predictions_vs_labels, \
                    true_pos, \
                    false_pos, \
                    true_neg, \
                    false_neg, ham = session.run([
                        validation_model.state, validation_model.loss, validation_model.accuracy,
                        validation_model.label_equality,
                        validation_model.true_pos, validation_model.false_pos,
                        validation_model.true_neg, validation_model.false_neg,
                        validation_model.ham],
                        {
                            validation_model.aspects: aspect,
            ...
            hamming /= len(validation_batches[0])
...
def train_aspect_polarity_model(training_batches, validation_batches, training_max_length, validation_max_length,
                                config):
    with tf.Session() as session:
        with tf.variable_scope("model", reuse=False):
        ...
        for epoch in range(config.max_epochs):
            ...
            for text_batch, annotation_batch, polarity_batch, length_batch, category_batch, pos_batch in zip(
                    *training_batches):
            ...
            for text_batch, annotation_batch, polarity_batch, length_batch, category_batch, pos_batch in zip(
                    *validation_batches):
                loss, accuracy, equalities = session.run(
                    [validation_model.loss, validation_model.accuracy, validation_model.equal_counts],
                    {
...
def load_iob_batches(train_datalist, validation_datalist, batch_size, mini_batch_size):
    ...
    try:
        training_batches, count, num_doc = train_datalist.create_iob_batches(train_datalist.iob_data, batch_size,
                                                                             mini_batch_size)
        validation_batches, _, _ = validation_datalist.create_iob_batches(validation_datalist.iob_data, batch_size,
                                                                          mini_batch_size)
    except:
    ...
    return training_batches, validation_batches, count, num_doc
...
def parse_arguments():
    ...
    common_parser.add_argument('validation_file', type=argparse.FileType('rb'),
                               help='Processed pickle file for validation')
    common_parser.add_argument('--batch-size', '-b', metavar='N', type=int, default=512,
                               help='size of training and validation batches')
    common_parser.add_argument('--hidden-neurons', '-n', metavar='N', type=int, default=125,
                               help='number of gru cell neurons')
    common_parser.add_argument('--input-dropout', metavar='N', type=float, default=1,
                               help='dropout retention rate applied to the input')
    common_parser.add_argument('--hidden-dropout', metavar='N', type=float, default=1,
                               help='dropout retention rate applied to bi-rnn gru cells')
    common_parser.add_argument('--learning-rate', '-l', metavar='N', type=float, default=0.001,
                               help='initial learning rate for the Adam optimizer')
    common_parser.add_argument('--max-epochs', '-m', metavar='N', type=int, default=1000,
                               help='maximum epochs before stopping training')
    common_parser.add_argument('--pos-embedding-size', '-p', metavar='N', type=int, default=0,
                               help='size of part of speech (POS) embedding vectors - 0 to disable')
    common_parser.add_argument('--save-model', '-s', metavar='PATH', type=str, default=None,
                               help='path to a model file the best model is saved to. Defaults to not saving')
    ...
    polarity_parser.add_argument('--word-embedding-size', '-w', metavar='N', type=int, default=200,
                                 help='size of input word embedding vectors')
    polarity_parser.add_argument('--distance-embedding-size', '-d', metavar='N', type=int, default=10,
                                 help='size of distance embedding vectors - 0 to disable')
    polarity_parser.add_argument('--category-embedding-size', '-c', metavar='N', type=int, default=10,
                                 help='size of category embedding vectors - 0 to disable')
...
if __name__ == '__main__':
    ...
    if arguments.model == "iob":
        config = IOBConfig(
            ...
            n_polarities = train_datalist.n_polarities
        )

        training_batches, validation_batches, count, num_doc = load_iob_batches(train_datalist, validation_datalist,
                                                                                config.batch_size,
                                                                                config.mini_batch_size)
        # ESC[2K clears the line
        print("\x1b[2KTraining started")
        train_iob_model(
            training_batches, validation_batches, train_datalist.max_len_sentences,
            validation_datalist.max_len_sentences, config, count, num_doc
        )
        ...
        train_aspect_polarity_model(
            training_batches, validation_batches, train_datalist.max_len_sentences,
            validation_datalist.max_len_sentences, config
        )
